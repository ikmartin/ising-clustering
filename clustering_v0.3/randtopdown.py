...
class TopDownRand(RefinementClustering):
...
def refine_with_lvec(model: TopDownRand, cluster: IsingCluster, weak=False):
...
def refine_with_avglvec(model: TopDownRand, cluster: IsingCluster, weak=False):
    """Refine the cluster only if the vector of the cluster doesn't satisfy one of the input levels in the cluster"""
    refine = False
...
def refine_with_qvec(model: TopDownRand, cluster: IsingCluster, weak=False):
...
def refine_with_sgn(model: TopDownRand, cluster: IsingCluster, weak=False):
...
def example7():
    ...
    model = TopDownRandQvec(circuit=circuit, weak=True)
    clusters = model.model()

    print_cluster_results(model, typename="TopDownQvecRand")
...
if __name__ == "__main__":
    """
    loop_models(
        ModelType=TopDownRandQvec, ModelName="TopDownQvecRand", loops=100, weak=False
    )
    loop_models(
        ModelType=TopDownRandSgn, ModelName="TopDownQvecSgn", loops=100, weak=False
    )"""
    loop_models(
        ModelType=TopDownRandLvec, ModelName="TopDownQvecSgn", loops=100, weak=True
    )
    loop_models(
        ModelType=TopDownRandAvgLvec, ModelName="TopDownQvecSgn", loops=100, weak=True
    )
