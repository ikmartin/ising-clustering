...
Let $\Circ(N,M)$ denote the set of all circuits $(N,M,f)$ and $\Ising(X)$ the set of all Ising systems $(X, H)$. In this section we develop a notion of isomorphic between circuits and between Ising systems.

\begin{defn}\label{defn:ising-symmetry}
  An \textbf{Ising circuit symmetry} or simply an \textbf{Ising symmetry} is a pair of maps $(\alpha, \beta)$ with $\alpha:\Circ(N,M) \to \Circ(N,M)$ and $\beta:\Ising(X) \to \Ising(X)$ such that
  \begin{align*}
    (X,H) ~ \text{ solves } ~ (N,M,f) ~ \Leftrightarrow~ \beta(X,H) ~ \text{ solves } ~ \alpha(N,M,f).
  \end{align*}
\end{defn}
...
\begin{example}\label{ex:spinaction}
  ...
  \end{align*}
  we obtain a map $\beta_\sigma: \Ising(X) \to \Ising(X)$. For any spin $s\in \Sigma^X$ we have
  \begin{align*}
    H(\sigma \cdot s) 
  ...
  \begin{align*}
    (X,H) ~ \text{ solves } ~ (N,M,f) ~\Leftrightarrow ~ \beta_\sigma (X,H) ~ \text{ solves } ~ \alpha_\sigma (N,M,f).
  \end{align*}
