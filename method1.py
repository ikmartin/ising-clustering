# Top Down Clustering
# refine_criterion uses qvec
# ties in clustering are broken randomly

# THIS CODE HAS A BUG IN REFINE
# When assigning points to clusters in the refinement, it computes the vdist of only the input spins,
# not the vdist of the input/output pair.
# Infuriatingly, however, this results in a better clustering.
# Note that the clustering it produces is still consistent, i.e. all input levels are satisfied by qvec or sgn or whatever
...
def example():
    ...
    N = 100
    for i in range(N):
        current_model = TopDownQvecFarthestPair(circuit=circuit)
        current_model.model()
        if len(model.clusters) > len(current_model.clusters):
            model = current_model

        print(f"Attempt {N} scored {len(current_model.clusters)} many clusters")
