...
class QNetwork(nn.Module):
    ...
    def forward(self, current_bans: torch.Tensor, current_coeffs: torch.Tensor):
        input_tensor = torch.cat([current_bans, current_coeffs], dim=-1).float()
...
class DQAgent():
    ...
    def best_actions(self, bans_batch, coeffs_batch):
        ...
        qualities = self.Q(bans_batch, coeffs_batch)
    ...
    def update_beliefs(self, samples):
        ...
        if torch.any(running):
            ...
            Y[running] += self.discount * best_qualities.detach()
        ...
        with torch.autograd.detect_anomaly():
            ...
            current_suggestions = self.Q(data['bans'], data['coeffs'])
            X = torch.diagonal(current_suggestions[..., data['action']])
