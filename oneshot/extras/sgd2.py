...
class VirtualSpinDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> torch.Tensor:
        ...
        if os.path.exists(cache_name):
            level, answer = torch.load(cache_name)
            return level.float(), answer
...
def cache_write(dataset: VirtualSpinDataset, index):
    ...
    if not os.path.exists(f'cache/{filename}'):
        input_level, answer = dataset.__getitem__(index)
        input_level = input_level.byte()
        torch.save((input_level, answer), filename)
