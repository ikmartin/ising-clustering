...
def make_correct_rows(n1, n2, n3, aux_array = None, included = None, desired = None, and_pairs = None, auxfix = False):
    N = n1 + n2 + n3
    num_inputs_1 = 1 << n1
    num_inputs_2 = 1 << n2
    num_inputs_3 = 1 << n3
    num_inputs = num_inputs_1 * num_inputs_2 * num_inputs_3
    num_fixed_columns = N
    ...
    correct_inps = torch.tensor([
            [i, j, k]
            for i, j, k in product(range(num_inputs_1), range(num_inputs_2), range(num_inputs_3))
        ])
    correct_inps = dec2bin(correct_inps, n1).to(torch.int8).reshape(num_inputs, 3*n1)
    outputs = torch.tensor([
            [i + j + k]
            for i, j, k in product(range(num_inputs_1), range(num_inputs_2), range(num_inputs_3))
        ])
    outputs = dec2bin(outputs, n1+2).to(torch.int8).reshape(num_inputs, n1+2)
    original_correct = torch.cat([correct_inps, outputs], dim=-1)

    
    mat = original_correct
    return mat, num_fixed_columns, n1+2
...
def constraints(n1, n2, n3, aux = None, degree = 2, radius = None, included = None, desired = None, and_pairs = None, ising=False, function_ands = None, auxfix = False, hyperplanes = None, bool_funcs = None):
    correct, num_fixed, num_free = make_correct_rows(n1, n2, n3, aux, included, desired, and_pairs, auxfix)
