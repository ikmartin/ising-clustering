...
def build_polynomial_fitter(circuit: PICircuit, degree: int) -> Solver:
    ...
    variable_keys = chain.from_iterable(
        [combinations(range(num_variables), i) for i in range(1, degree + 1)]
    )
    params = {key: solver.NumVar(-inf, inf, str(key)) for key in variable_keys}
                ...
                if s == correct_out_array[i]:
                    ...
                    for key, var in params.items():
                        constraint.SetCoefficient(
                            var,
                            prod_term(other_binary, key)
                            - prod_term(current_binary, key),
                        )
